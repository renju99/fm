...
export class FacilitiesAssetPerformanceDashboard extends Component {
    setup() {
        // Handle props defensively
        if (this.props.action !== undefined) {
            console.log("Facilities Asset Performance Dashboard: action received:", this.props.action);
            // Extract actionId from action object if needed
            if (this.props.action.id) {
                console.log("Facilities Asset Performance Dashboard: action ID:", this.props.action.id);
            }
        }
    ...
    ensureScrolling() {
        ...
        const container = document.querySelector('.o_asset_performance_dashboard');
        if (container) {
        ...
        // Target all possible Odoo parent containers
        ...
        console.log('Scrolling enforcement applied to dashboard');
    }
    ...
    async loadDashboardData() {
        try {
            ...
            console.log("Facilities Asset Performance Dashboard: Loading data for period:", this.state.selectedPeriod);
            
            // Prepare parameters for the RPC call
            const params = [this.state.selectedPeriod, this.state.selectedCategory];
            ...
            if (data && !data.error) {
                console.log("Facilities Asset Performance Dashboard: Successfully loaded data:", data);
                ...
                console.error("Facilities Asset Performance Dashboard: Server returned error:", data?.error);
            ...
            console.error("Facilities Asset Performance Dashboard: Error loading data:", error);
    ...
    _handleError(error) {
        ...
        if (this.retryCount < this.maxRetries && this._isRetryableError(error)) {
            ...
            console.log(`Facilities Asset Performance Dashboard: Retrying in ${retryDelay}ms (attempt ${this.retryCount}/${this.maxRetries})`);
            
            this.retryTimeout = setTimeout(() => {
                this.loadDashboardData();
            }, retryDelay);
        } else if (this.retryCount >= this.maxRetries) {
            console.error("Facilities Asset Performance Dashboard: Max retries reached");
            this.notification.add(
    ...
    _isRetryableError(error) {
        ...
        const nonRetryableErrors = [
            'cost', // The specific error we're seeing
            'permission',
            'validation',
            'not found'
        ];
    ...
    getDefaultData() {
        return {
            ...
            metrics: {
                ...
                workorder_completion_rate: 0.0,
                avg_workorder_duration: 0.0,
    ...
    renderCharts() {
        ...
        // ROI Trend Chart
        if (trends.roi_trend && trends.roi_trend.length > 0) {
            this.renderROIChart(trends.roi_trend);
        }
        
        // Utilization Trend Chart
        if (trends.utilization_trend && trends.utilization_trend.length > 0) {
            this.renderUtilizationChart(trends.utilization_trend);
        }
        
        // Maintenance Cost Trend Chart
        if (trends.maintenance_cost_trend && trends.maintenance_cost_trend.length > 0) {
            this.renderMaintenanceCostChart(trends.maintenance_cost_trend);
        }
    }

    renderROIChart(data) {
        // Simple chart rendering - in a real implementation, you might use Chart.js or similar
        console.log("Rendering ROI Chart with data:", data);
    }

    renderUtilizationChart(data) {
        console.log("Rendering Utilization Chart with data:", data);
    }

    renderMaintenanceCostChart(data) {
        console.log("Rendering Maintenance Cost Chart with data:", data);
    }
    ...
    async onCategoryChange(category) {
        ...
        this.retryCount = 0; // Reset retry count for new category
        await this.loadDashboardData();
    }
    ...
    exportData() {
        ...
        if (this.state.selectedPeriod === 'custom_range' && this.state.customDateFrom && this.state.customDateTo) {
            filename = `facilities_asset_performance_report_${this.state.customDateFrom}_to_${this.state.customDateTo}.csv`;
        } else {
            filename = `facilities_asset_performance_report_${this.state.selectedPeriod}.csv`;
        }
...
FacilitiesAssetPerformanceDashboard.template = 'facilities_asset_performance_dashboard_template';

// Register the dashboard component
registry.category("actions").add("facilities_asset_performance_dashboard", FacilitiesAssetPerformanceDashboard);