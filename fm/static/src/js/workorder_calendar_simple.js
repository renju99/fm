/* Enhanced Work Order Calendar Functionality - Simple Version */

console.log('Work Order Calendar JavaScript (Simple) loaded successfully');
...
function initializeCalendarEnhancements() {
    ...
    setTimeout(() => {
        ...
        // Re-run enhancements periodically to catch dynamically loaded content
        setInterval(() => {
            enhanceCalendarEvents();
        }, 5000);
    }, 1000);
...
function enhanceCalendarEvents() {
    const events = document.querySelectorAll('.fc-event:not([data-enhanced])');
    console.log(`Found ${events.length} unenhanced calendar events`);
